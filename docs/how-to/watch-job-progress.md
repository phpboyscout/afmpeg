...
- **Phase B, the engine side-channel** (a **v9+** ffmpeg-wasi engine, WASM backend). When the module
...
When both sources are live the **engine's is authoritative**, because it measures the work that actually
...
## Back-pressure, and why you cannot slow the job down
...
- **Byte-only fallback.** Without a phase-B duration (a pre-v9 engine, or the native backend below)
  ...
  inputs** (e.g. an MP4 whose index sits at the end) make it lumpy, and it is clamped so it never goes
