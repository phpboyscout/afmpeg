...
|---|---|
| `-b:v 300k` | `Options{"b": "300k"}`, because the CLI parses the `:v` itself and libav never sees it |
| `-crf 23` | `Options{"crf": "23"}`, the same name |
...
`SubtitleCodec` and name the source stream in `Map`, and the packets pass through untouched, with
