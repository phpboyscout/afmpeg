...
| both `Duration` and `End` on one output | rejected in Go, since they are mutually exclusive |
...
| `libx264` on an **lgpl** module | rejected by the engine, so use `libopenh264` |
