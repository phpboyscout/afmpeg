...
| module source | none, so `New` returns `ErrNoModule` | one is mandatory |
...
| `WithSHA256` | none, so a `WithModuleURL` download is **not** verified | always set it |
...
`WithBackend(b)` replaces the WASM backend with your own, and in practice the
...
| `WithHTTPClient(c)` | `http.DefaultClient` | The client used for the download. The default has **no timeout**, so the fetch is bounded only by `New`'s context. |
...
| `WithReleaseBaseURL(url)` | `https://gitlab.com/api/v4/projects/83847809/packages/generic/ffmpeg-wasi` | Fetch from a mirror or internal store. The signature is over content, so the URL is untrusted input and verification is unchanged. |
...
means **the WKD cross-check does not run**, and the embedded key is the only anchor.
