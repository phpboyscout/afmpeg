...
func rd(c net.Conn, n int) []byte { b := make([]byte, n); if _, err := io.ReadFull(c, b); err != nil { return nil }; return b }
...
func serve(c net.Conn) {
	defer c.Close()
	hdr := rd(c, 6)
	if hdr == nil || hdr[0] != 'O' { return }
	mode := hdr[1]
	name := string(rd(c, int(u32(hdr[2:]))))
	mu.Lock()
	f := files[name]
	if f == nil && mode == 'w' { f = &memFile{}; files[name] = f }
	mu.Unlock()
	if f == nil { c.Write([]byte{1}); return }
	if mode == 'w' { f.data = f.data[:0] }
	c.Write([]byte{0})
	var off, maxW int64
	for {
		op := rd(c, 1)
		...
		case 'R':
			...
			if off < int64(len(f.data)) {
				end := off + int64(n); if end > int64(len(f.data)) { end = int64(len(f.data)) }
				chunk = f.data[off:end]
			}
			binary.Write(c, binary.LittleEndian, uint32(len(chunk)))
			c.Write(chunk); off += int64(len(chunk))
		case 'W':
			n := int(u32(rd(c, 4)))
			buf := rd(c, n)
			atomic.AddInt64(&writes, 1)
			need := off + int64(n)
			if need > int64(len(f.data)) { f.data = append(f.data, make([]byte, need-int64(len(f.data)))...) }
			copy(f.data[off:], buf); off += int64(n)
			if off > maxW { maxW = off }
			binary.Write(c, binary.LittleEndian, uint32(n))
		case 'S':
			target := int64(u64(rd(c, 8))); whence := rd(c, 1)[0]
			atomic.AddInt64(&seeks, 1)
			switch whence {
			case 0: off = target
			case 1: off += target
			case 2: off = int64(len(f.data)) + target
			}
			if mode == 'w' && off < maxW { atomic.AddInt64(&backSeeks, 1) }
...
func boxes(tag string, d []byte) {
	find := func(s string) int { for i := 0; i+4 <= len(d); i++ { if string(d[i:i+4]) == s { return i } }; return -1 }
	ftyp, mdat, moov, moof := find("ftyp"), find("mdat"), find("moov"), find("moof")
	kind := "??"
	if moof >= 0 { kind = "FRAGMENTED (moof present)" } else if moov > mdat && mdat >= 0 { kind = "non-fragmented (moov after mdat)" } else if moov >= 0 { kind = "non-fragmented (moov before mdat)" }
	fmt.Printf("  %-8s %d bytes | ftyp@%d mdat@%d moov@%d moof@%d -> %s\n", tag, len(d), ftyp, mdat, moov, moof, kind)
}

func main() {
	sock := "/tmp/spike.sock"; os.Remove(sock)
	...
	ln, err := net.Listen("unix", sock); if err != nil { panic(err) }
	go func() { for { c, e := ln.Accept(); if e != nil { return }; go serve(c) } }()
	...
	mu.Lock(); out := files["out.mp4"]; mu.Unlock()
	if out == nil || len(out.data) == 0 { fmt.Println("FAIL: no output produced in the store"); os.Exit(1) }
	...
	if backSeeks > 0 { fmt.Println("  ✅ seekable custom-AVIO wrote a NON-FRAGMENTED MP4 via backward seeks — the case HTTP PUT cannot do") }
}
