...
#
# PRE-1.0 POLICY: afmpeg is v0.x. Breaking changes are permitted and ship
# as a MINOR bump. This script is therefore ADVISORY only — it always exits
# 0 so it never fails a build or an MR pipeline. Its job is to make API
# changes *visible* so they are seen and intentional. From v1.0 the wrapping
# CI job becomes a BLOCKING gate (see .gitlab-ci.yml).
#
# Usage:
#   scripts/apidiff.sh            # compare against the latest vX.Y.Z tag
#   scripts/apidiff.sh v0.1.0     # compare against an explicit baseline tag
#
